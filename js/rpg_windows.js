...
Window_Base.prototype.drawActorClass = function(actor, x, y, width) {
    width = width || 168;
    this.resetTextColor();
    this.drawText(actor.currentClass().name, x+20, y, width, 'right');
};
...
Window_Base.prototype.drawCurrentAndMax = function(current, x, y,
                                                   width, color1, color2) {
...
Window_Base.prototype.drawActorHp = function(actor, x, y, width) {
	if(actor == $gameActors.actor(5)){}
	else{
		width = width || 186;
		var color1 = this.hpGaugeColor1();
		var color2 = this.hpGaugeColor2();
		var val = 5;
		if(actor == $gameActors.actor(4)) {val = 4;}
		if(actor == $gameActors.actor(3)) {val = 3;}
		if(actor == $gameActors.actor(2)) {val = 2;}
		if(actor == $gameActors.actor(1)) {val = 1;}
		//this.drawGauge(x, y, width, actor.hpRate(), color1, color2);
		
	//改rate + 血條/生命值/hp值
		var a = $gameVariables.value(val);
		if(a<=0) a=0;
		var b = 100;
		var rateAB = a/b;
		
		this.drawGauge(x, y, width, rateAB, color1, color2);
		this.changeTextColor(this.textColor(27));
		this.drawText('好感度♥ ', x, y, 100);
		//this.drawText('好感度♥ '+TextManager.hpA, x+15, y, 65);
		//this.drawCurrentAndMax(actor.hp, x+55, y, width,
							   //this.hpColor(actor), this.normalColor());
		this.drawCurrentAndMax($gameVariables.value(val), x+55, y, width,
							   this.hpColor(actor), this.normalColor());
	}
};
...
Window_Base.prototype.drawActorMp = function(actor, x, y, width) {
    ...
    this.drawCurrentAndMax(actor.mp, x, y, width,
...
Window_Base.prototype.drawActorSimpleStatus = function(actor, x, y, width) {
	...
	//if(actor == $gameActors.actor(5)){actor = $gameActors.actor(1);}
    this.drawActorName(actor, x, y);
    //this.drawActorLevel(actor, x, y + lineHeight * 1);
    this.drawActorIcons(actor, x, y + lineHeight * 2);
    this.drawActorClass(actor, x2, y);
    this.drawActorHp(actor, x2-180, y + lineHeight * 1.5, width);
    //this.drawActorMp(actor, x2-180, y + lineHeight * 2, width);
};
...
Window_Gold.prototype.refresh = function() {
    ...
    this.contents.clear();
	this.changeTextColor(this.textColor(27));
	this.drawText('距離死亡還有', x, 0, width,'center');
   // this.drawCurrencyValue(this.value(), this.currencyUnit(), x, 0, width);
    this.drawText(this.value(), x, this.fittingHeight(1)/2, width,'center');
};

Window_Gold.prototype.value = function() {
    //return $gameParty.gold();
	return $gameVariables.value(20)+' 日';
};
...
Window_Message.prototype.createSubWindows = function() {
    ...
    this._goldWindow.x = Graphics.boxWidth;
